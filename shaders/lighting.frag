...
void main (void){
    if (!enablelighting){
        ...
        mat3 mview = mat3(modelview);
        vec3 n = normalize(transpose(inverse(mview)) * normal);
        ...
        for(int j = 0; j < nlights; j++) {
           ...
           vec4 cursum = ambient + (diffuse * max(dot(n, l), 0)) + (specular * pow(max(dot(n, h), 0), shininess));
           sum = sum + cursum;
           sum = sum * lightcolors[j];
        }
