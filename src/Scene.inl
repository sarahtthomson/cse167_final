...
// Defines a M_model matrix
mat3 create_M(float u_1, float u_2, float u_3) {
    mat3 M_model;
    M_model[0][0] = u_1;
    M_model[1][1] = u_2;
    M_model[2][2] = u_3;
...
void Scene::init(void) {
    // M_model for bunny
    M_model = create_M(4.0f, 1.0f, 2.0f);

    // initiate angular velocity of world coordinates
    w = vec3(0.1f, 0.0f, 2.0f);
    ...
    // initiate angular velocity of model coordinates
    //vec3 Omega(w);
    vec3 Omega(inverse(R_mat3) * w);

    // Ekinetic ellipsoid vector
    float E_k = dot(Omega, (M_model * Omega));

    // Angular Momentum vector
    float M = dot(Omega, (transpose(M_model) * M_model) * Omega);
    //vec3 M = M_model * Omega;
    ...
    node["MPoin"]->models.push_back(model["sphere2"]);
    node["MPoin"]->modeltransforms.push_back(scale(vec3(sqrt(M) / M_model[0][0], sqrt(M) / M_model[1][1], sqrt(M) / M_model[2][2])));

    node["polhode"]->models.push_back(model["sphere3"]);
    node["polhode"]->modeltransforms.push_back(scale(vec3(0.125f)));

    node["spinny"]->models.push_back(model["bunny"]);
    ...
    node["world"]->childnodes.push_back(node["MPoin"]);
    node["world"]->childtransforms.push_back(mat4(1.0f));
